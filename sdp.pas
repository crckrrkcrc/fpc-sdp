...
begin
  ...
  // Ищем формат с таким payload
  for i := 0 to High(M.Format) do
  begin
    if M.Format[i].Payload = Payload then
    begin
      case A.Name of
        'rtpmap': ParseRtpMap(M.Format[i], Parts[1]);
        'rtcp-fb':
          begin
            SetLength(M.Format[i].Feedback, Length(M.Format[i].Feedback) + 1);
            M.Format[i].Feedback[High(M.Format[i].Feedback)] := Parts[1];
          end;
        'fmtp':
          begin
            SetLength(M.Format[i].Params, Length(M.Format[i].Params) + 1);
            M.Format[i].Params[High(M.Format[i].Params)] := Parts[1];
          end;
      end;
      Exit;
    end;
  ...
  // Если формат не найден, создаем новый
  SetLength(M.Format, Length(M.Format) + 1);
  M.Format[High(M.Format)].Payload := Payload;
  
  case A.Name of
    'rtpmap': ParseRtpMap(M.Format[High(M.Format)], Parts[1]);
    'rtcp-fb':
      begin
        SetLength(M.Format[High(M.Format)].Feedback, 1);
        M.Format[High(M.Format)].Feedback[0] := Parts[1];
      end;
    'fmtp':
      begin
        SetLength(M.Format[High(M.Format)].Params, 1);
        M.Format[High(M.Format)].Params[0] := Parts[1];
      end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        M.Format[High(M.Format)].Name := ''; // Явно инициализируем имя
      end;
    end;
  end;
end;
